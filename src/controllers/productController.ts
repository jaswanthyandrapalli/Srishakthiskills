...
const useFallbackData = () => mongoose.connection.readyState !== 1;

const applyProductFilters = (items: any[], query: Record<string, any>) => {
  ...
  if (search) {
    ...
    results = results.filter((item) =>
      [item.name, item.description, item.fabric]
        .filter(Boolean)
...
export const getProducts = async (req: Request, res: Response): Promise<void> => {
  try {
    if (useFallbackData()) {
      res.json(applyProductFilters(fallbackProducts, req.query as Record<string, any>));
      return;
    }
...
// @access  Private
export const createProductReview = async (req: any, res: Response): Promise<void> => {
  try {
